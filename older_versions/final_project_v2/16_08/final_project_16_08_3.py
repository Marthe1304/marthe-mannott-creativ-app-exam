...
#def generate_empty_decay_list(length_xy_axis, image):
 #   empty_decay_list = []
  #  for n in range(length_xy_axis[0] * length_xy_axis[1]):
    #    if image[n] == 0:
   #         empty_decay_list.append(random.randint(0, 10))
     #   else:
...
def generate_image(image_list, appending_list, weight, decay_list):
    ...
    for n in range(len(image_list)):
        ...
        if image_list[n] == 0:
            ...
            if appending_list[n] < (3 + weight*3):
                new_image_list.append(0)
            else:
                new_image_list.append(1)
...
def generate_new_decay_list_2(old_decay_list, old_image_list, new_image_list):
    new_decay_list = []
    for n in range(len(old_decay_list)):
        if old_image_list[n] != new_image_list[n]:
            new_decay_list.append(20)
...
image_list, length_xy_axis, width, height = convert_image_to_list("Mona_Lisa.jpg", 4, 128)
...
for n in range(200):
    ...
    new_image = convert_list_to_image(image_list, length_xy_axis, True, width, height)
    new_image.save(f"/Users/macmannott/Documents/creative_applications/final_project_v2/16_08/Images/image_{n+1}.png")
    print(n)
