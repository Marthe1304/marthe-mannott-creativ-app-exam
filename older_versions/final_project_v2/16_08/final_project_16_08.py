...
def generate_image(image_list, appending_list, weight, age_list):
    ...
    for n in range(len(image_list)):
        if decay_list[n] == 0:
            if image_list[n] == 1:
                if appending_list[n] < 8 - (2 + (1 - weight) * 1):
                    new_image_list.append(0)
                else:
                    new_image_list.append(1)
            else:
                if appending_list[n] > 2 + weight * 1:
                    new_image_list.append(1)
                else:
                    new_image_list.append(0)
        else:
...
def generate_new_decay_list(old_decay_list, old_image_list, new_image_list):
    ...
    for n in range(len(old_decay_list)):
        if old_image_list[n] != new_image_list[n]:
            new_decay_list.append(100) #between 5 and 100
        else:
...
image_list, length_xy_axis, width, height = convert_image_to_list("Mona_Lisa.jpg", 2, 80)
