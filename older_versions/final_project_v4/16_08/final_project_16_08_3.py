...
def generate_image(image_list, appending_list, weight, decay_list):
    ...
    for n in range(len(image_list)):
        value = (
            ...
            + (weight-0.5) * (-25) * ((math.cos(((appending_list[n]+0.7)*4.255)/math.pi) - math.cos(((appending_list[n]+0.7)*12.765)/math.pi)) * (-0.55) + 0.5) * (((appending_list[n]-4)**2)**0.5) *0.33
            ...
            #+ decay_list[n]*(image_list[n] - 0.5) * (-0.2) * (((((appending_list[n]-4)**4)/(-128)) + (((appending_list[n]-4)**2)/8))*2)
...
image_list, length_xy_axis, width, height = convert_image_to_list("Mona_Lisa.JPG", 4, 145)
