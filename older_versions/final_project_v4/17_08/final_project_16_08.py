...
def generate_image(image_list, appending_list, weight, decay_list):
    ...
    for n in range(len(image_list)):
        value = (
            appending_list[n]
            + (weight-0.5) * (-10)
            * ((math.cos(((appending_list[n]+0.5)*5)/math.pi) - 0.2 * math.cos(((appending_list[n]+0.5)*15)/math.pi)) * (-0.6) + 0.5)
...
image_list, length_xy_axis, width, height = convert_image_to_list("Mona_Lisa.JPG", 4, 80)
