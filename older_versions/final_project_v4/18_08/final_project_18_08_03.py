...
def generate_image(image_list, appending_list, weight, decay_list, weight_factor, decay_factor):
    ...
    for n in range(len(image_list)):
        value = round(
            appending_list[n]
            + (weight - 0.5) * (-20) * (((appending_list[n]-4)**2)/(16))
            #+ (weight - 0.5) * (-1.4 ) #* weight_factor * (((appending_list-3.4)**2)/(-11.56)+1)
            + (image_list[n]-0.5) * (-2) * decay_list[n] * (0.2) * decay_factor
        , 1)

        if appending_list[n] == 8 and image_list[n] == 1:
...
def check_values(fp_image, iterations, pixelate_factor, white_threshold, rescale, weight_factor, decay_factor):
...
def infinity_morpher(fp_image, iterations, pixelate_factor, white_threshold, rescale, weight_factor, decay_factor):

    check_values(fp_image, iterations, pixelate_factor, white_threshold, rescale, weight_factor, decay_factor)
...
infinity_morpher("/Users/macmannott/Documents/creative_applications/final_project_v4/18_08/Mona_Lisa.jpg", 250, 4, 10, True, 1, 0)