            ...
            if y == 0:
                if x == 0:
                    appending_list.append(
                        image[length_xy_axis[0] * length_xy_axis[1] - 1] * 0.75
                        + image[(length_xy_axis[1] - 1) * length_xy_axis[0]]
                        + image[(length_xy_axis[1] - 1) * length_xy_axis[0] + 1] * 0.75

                        + image[length_xy_axis[0] - 1]
                        + image[1]

                        + image[2 * length_xy_axis[0] - 1] * 0.75
                        + image[length_xy_axis[0]]
                        + image[length_xy_axis[0] + 1] * 0.75
                    )

                elif x == (length_xy_axis[0] - 1):
                    appending_list.append(
                        image[(length_xy_axis[1] - 1) * length_xy_axis[0] + (x - 1)] * 0.75
                        + image[(length_xy_axis[1] - 1) * length_xy_axis[0] + x]
                        + image[(length_xy_axis[1] - 1) * length_xy_axis[0]] * 0.75

                        + image[x - 1]
                        + image[0]

                        + image[length_xy_axis[0] + (x - 1)] * 0.75
                        + image[length_xy_axis[0] + x]
                        + image[length_xy_axis[0]] * 0.75
                    )
                ...
                if x == 0:
                    appending_list.append(
                        image[(y - 1) * length_xy_axis[0] + (length_xy_axis[0] - 1)] * 0.75
                        + image[(y - 1) * length_xy_axis[0]]
                        + image[(y - 1) * length_xy_axis[0] + 1] * 0.75
                    ...
                    appending_list.append(
                        image[(y - 1) * length_xy_axis[0] + (x - 1)] * 0.75
                        + image[(y - 1) * length_xy_axis[0] + x]
                        + image[(y - 1) * length_xy_axis[0]] * 0.75
...
def generate_image(image_list, appending_list, weight, decay_list):
    ...
    for n in range(len(image_list)):
        value = (
            appending_list[n]
            #+ decay_list[n] * (image_list[n] - 0.5) * (-0.6)
            #+ (weight-0.5) * (-10)




            + (weight-0.5) * (-10)* ((math.cos(((appending_list[n]+0.5)*5)/math.pi) - 0.2 * math.cos(((appending_list[n]+0.5)*15)/math.pi)) * (-0.6) + 0.5)

            # * ((math.cos(((appending_list[n])*5.714)/math.pi) - 0.2 * math.cos(((appending_list[n])*17.142)/math.pi)) * (-0.6) + 0.5)
            #+ (weight - 0.5) * (-25) * (((((appending_list[n]-4)**4)/(-128)) + (((appending_list[n]-4)**2)/8))*2)
            #+ (weight-0.5) * (-25) * (((math.cos(5*(appending_list[n]))/ math.pi) * (-0.5) + 0.5))
            #+ (weight-0.5) * (-25) * (((math.cos(4*(appending_list[n]+1))/ math.pi) * (-0.5) + 0.5))
            #+ (weight-0.5) * (-25) * ((((appending_list[n]-4)**2)/16))

            + decay_list[n] * (image_list[n] - 0.5) * (-0.2) * ((math.cos(((appending_list[n]) * 5.714) / math.pi) - 0.2 * math.cos(((appending_list[n]) * 17.142) / math.pi)) * (-0.6) + 0.5)

            #+ decay_list[n] * (image_list[n] - 0.5) * (-0.2)#-0.2
            #+ decay_list[n]*(image_list[n] - 0.5) * (-0.2) * (((((appending_list[n]*(8/7)-4)**4)/(-128)) + (((appending_list[n]*(8/7)-4)**2)/8))*2)
            #+ decay_list[n] * (image_list[n] - 0.5) * (-0.2) * ((math.cos(((appending_list[n]-0.5) * 6.666) / math.pi) - 0.2 * math.cos(((appending_list[n]-0.5) * 20) / math.pi)) * (1) + 0.5)
            #+ decay_list[n] * (image_list[n] - 0.5) * (-0.2) * (((appending_list[n] - 4) ** 4) / (-256) + 1)
        )
        if appending_list[n] == 7:
...
def generate_new_decay_list(old_decay_list, old_image_list, new_image_list, appending_list):
    new_decay_list = []
    for n in range(len(old_decay_list)):
        if old_image_list[n] != new_image_list[n]:
            new_decay_list.append(0)
        else:
            new_decay_list.append(old_decay_list[n]+round((((appending_list[n]-3.9)**2)**0.5)/(-3.5), 2)+1)
...
image_list, length_xy_axis, width, height = convert_image_to_list("test.jpeg", 8, 30)
print(length_xy_axis)
new_image = convert_list_to_image(image_list, length_xy_axis, True, width, height)
new_image.save(f"/Users/macmannott/Documents/creative_applications/final_project_v4/18_08/Images/image_0.png")
...
for n in range(400):
    ...
    appending_list = generate_appending_list(image_list, length_xy_axis)
    decay_list = generate_new_decay_list(decay_list, image_list, new_image_list, appending_list)
    image_list = new_image_list

    new_image = convert_list_to_image(image_list, length_xy_axis, True, width, height)
    new_image.save(f"/Users/macmannott/Documents/creative_applications/final_project_v4/18_08/Images/image_{n+1}.png")
    print(n)
    print(weight)